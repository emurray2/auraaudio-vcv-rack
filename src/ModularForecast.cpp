...
struct ModularForecastDisplay : OpenGlWidget {
    ModularForecast *module = new ModularForecast();
    ...
    void displayParticles(void)
    {
        ...
        for (i = 0; i < currentParticles; i++)
        {
            ...
            Particles[i].XCoor += Particles[i].dX * speedChange * (1 + (5 * module->snowVoltage)); //Logic for moving the particle
            Particles[i].YCoor += Particles[i].dY * speedChange * (1 + (5 * module->snowVoltage));
